...
#define TEST_FONT_COUNT 32U
#define TEST_CONSOLE_FD 41
...
main(int argc KBD_ATTR_UNUSED, char **argv KBD_ATTR_UNUSED)
{
	...
	if (kfont_init("libkfont-test06", &ctx) != 0)
	...
	if (kfont_put_font(ctx, TEST_CONSOLE_FD, buf, TEST_FONT_COUNT, 8, 16, 32) == 0)
		kbd_error(EXIT_FAILURE, 0, "kfont_put_font unexpectedly succeeded");

	for (i = 0; i < sizeof(buf); i++) {
		if (buf[i] != (unsigned char)((i + 1) & 0xff))
