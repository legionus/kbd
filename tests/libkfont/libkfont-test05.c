#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <errno.h>

#include <linux/kd.h>

#include "kfontP.h"
#include "libcommon.h"

#define TEST_FONT_COUNT 17U
#define TEST_CONSOLE_FD 37

static unsigned char *expected_buf;
static unsigned int expected_count;

static int
fake_ioctl(int fd, unsigned long req, void *arg)
{
	if (fd != TEST_CONSOLE_FD)
		kbd_error(EXIT_FAILURE, 0, "unexpected fd: %d", fd);

	if (req == KDGETMODE) {
		unsigned int *mode = arg;

		*mode = KD_TEXT;
		return 0;
	}

	if (req == KDFONTOP) {
		struct console_font_op *op = arg;
		size_t used = 32U * expected_count;
		size_t i;

		if (op->op != KD_FONT_OP_SET)
			kbd_error(EXIT_FAILURE, 0, "unexpected font op: %u", op->op);

		if (op->width != 8)
			kbd_error(EXIT_FAILURE, 0, "unexpected width: %u", op->width);

		if (op->height != 16)
			kbd_error(EXIT_FAILURE, 0, "unexpected height: %u", op->height);

		if (op->charcount == expected_count) {
			if (op->data != expected_buf)
				kbd_error(EXIT_FAILURE, 0, "unexpected source buffer on first attempt");

			errno = EINVAL;
			return -1;
		}

		if (op->charcount != 256)
			kbd_error(EXIT_FAILURE, 0, "unexpected retried charcount: %u", op->charcount);

		if (op->data == expected_buf)
			kbd_error(EXIT_FAILURE, 0, "retry did not use a copied buffer");

		for (i = 0; i < used; i++) {
			if (op->data[i] != expected_buf[i])
				kbd_error(EXIT_FAILURE, 0, "unexpected copied byte at %zu: %#x", i, op->data[i]);
		}

		for (; i < 32U * 256U; i++) {
			if (op->data[i] != 0)
				kbd_error(EXIT_FAILURE, 0, "unexpected padded byte at %zu: %#x", i, op->data[i]);
		}

		return 0;
	}

	kbd_error(EXIT_FAILURE, 0, "unexpected request: %lu", req);
	return -1;
}

int
main(int argc KBD_ATTR_UNUSED, char **argv KBD_ATTR_UNUSED)
{
	struct kfont_context *ctx;
	struct kfont_ops ops;
	unsigned char buf[TEST_FONT_COUNT * 32];
	size_t i;

	if (kfont_init("libkfont-test05", &ctx) != 0)
		kbd_error(EXIT_FAILURE, 0, "unable to create kfont context");

	kfont_set_logger(ctx, NULL);

	for (i = 0; i < sizeof(buf); i++)
		buf[i] = (unsigned char)(i & 0xff);

	expected_buf = buf;
	expected_count = TEST_FONT_COUNT;

	ops = ctx->ops;
	ops.ioctl_fn = fake_ioctl;
	kfont_set_ops(ctx, &ops);

	if (kfont_put_font(ctx, TEST_CONSOLE_FD, buf, expected_count, 8, 16, 32) != 0)
		kbd_error(EXIT_FAILURE, 0, "kfont_put_font failed");

	for (i = 0; i < sizeof(buf); i++) {
		if (buf[i] != (unsigned char)(i & 0xff))
			kbd_error(EXIT_FAILURE, 0, "source buffer changed at %zu: %#x", i, buf[i]);
	}

	kfont_free(ctx);

	return EXIT_SUCCESS;
}
