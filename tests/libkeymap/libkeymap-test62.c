...
main(int argc KBD_ATTR_UNUSED, char **argv KBD_ATTR_UNUSED)
{
	...
	init_test_keymap(&keymap, "xkb-group-fallback");
	set_xkb_config_root();
	set_xkb_suppress_warnings();

	if (convert_xkb_keymap(keymap.ctx, &params) != 0)
		kbd_error(EXIT_FAILURE, 0, "Unable to convert XKB us,empty layout");
	...
	expect_key_symbol(keymap.ctx, 1 << KG_SHIFTL, 16, "q");
	expect_key_symbol(keymap.ctx, (1 << KG_SHIFTL) | (1 << KG_SHIFT), 16, "Q");
