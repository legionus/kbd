...
#define TEST_KEY_INDEX 4
#define TEST_KEY_VALUE K(KT_LATIN, 'p')
...
main(int argc KBD_ATTR_UNUSED, char **argv KBD_ATTR_UNUSED)
{
	...
	if (lk_add_key(ctx, TEST_KEY_TABLE, TEST_KEY_INDEX, TEST_KEY_VALUE) != 0)
		kbd_error(EXIT_FAILURE, 0, "Unable to add key for EPERM test");
	...
	if (lk_load_keymap(ctx, TEST_CONSOLE_FD, K_XLATE) == 0)
		kbd_error(EXIT_FAILURE, 0, "EPERM did not abort keymap load");
	...
	if (saw_function_ioctls)
		kbd_error(EXIT_FAILURE, 0, "Function strings were loaded after EPERM");

	if (saw_diacr_ioctls)
		kbd_error(EXIT_FAILURE, 0, "Compose table was loaded after EPERM");
